...
# --- API Key (Replace with your actual key or environment variable) ---
# IMPORTANT: For production, store this in an environment variable, not directly in code.
API_KEY = "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" # Replace with your actual key!
...
try:
    ...
    knn_model = None # Set to None so the app can still run, but without prediction

label_map = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'K', 'L', 'M', 'N', 'O', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'W', 'X', 'Y', 'Z', 'del', 'nothing', 'space']

# --- Global state variables for communication with frontend ---
# These variables will be updated by the video processing thread and accessed by API endpoints
global_predicted_sign = "Initializing..."
global_collected_letters = []
global_autocorrected_sentence = ""
global_ai_response_text = ""
global_fps = 0
global_hand_detected = False
global_letter_timeline = [] # Stores {letter, timestamp} objects

# --- Buffer for predictions (for autocorrection logic) ---
buffer_time = 3  # seconds (reduced for faster demo)
prediction_buffer = deque()
last_buffer_process_time = time.time()

# --- Webcam setup ---
cap = None # Will be initialized/released by API calls
video_stream_active = False
video_processing_thread = None
frame_lock = threading.Lock() # Lock to protect global_variables during frame processing

def get_small_talk_reply(prompt):
    """Fetches a reply from the DeepSeek AI based on the prompt."""
    if not API_KEY or API_KEY == "YOUR_DEEPSEEK_API_KEY_HERE":
        print("API Key not configured. Please update API_KEY in app.py")
        ...
        "messages": [
            {
                ...
                "content": (
                    ...
                    "If the prompt is unrelated, politely refuse or state you cannot help with that."
                    "Keep responses concise and directly answer the question based on the input."
                )
            },
            {"role": "user", "content": prompt}
        ],
        "temperature": 0.7,
        "stream": False # Ensure non-streaming for simplicity in this example
    }
    try:
        ...
        print(f"Error getting reply from DeepSeek API: {e}")
        return f" Error getting reply from AI: {e}"
    except Exception as e:
        print(f"Unexpected error parsing AI reply: {e}")
        return " An unexpected error occurred with the AI response."

def process_video_stream():
    """Continuously processes video frames from the webcam."""
    global cap, global_predicted_sign, global_collected_letters, global_autocorrected_sentence, \
           global_fps, global_hand_detected, last_buffer_process_time, prediction_buffer, \
           video_stream_active, global_letter_timeline

    prev_frame_time = time.time()

    while video_stream_active and cap and cap.isOpened():
        ret, frame = cap.read()
        if not ret:
            print("Failed to grab frame from webcam. Ending stream processing.")
            break

        frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        results = hands.process(frame_rgb)

        local_predicted_sign = "No Hand Detected"
        local_hand_detected = False

        if results.multi_hand_landmarks:
            local_hand_detected = True
                    ...
                    try:
                        ...
                        if isinstance(prediction, (int, np.integer)):
                            if 0 <= prediction < len(label_map):
                                local_predicted_sign = label_map[prediction]
                            else:
                                local_predicted_sign = "Prediction Index Out of Bounds"
                        elif isinstance(prediction, str):
                            local_predicted_sign = prediction
                        else:
                            local_predicted_sign = f"Unknown prediction type: {type(prediction)}"
                    except Exception as e:
                        print(f"Prediction error: {e}")
                        local_predicted_sign = "Prediction Error"
                else:
                    local_predicted_sign = "Model not loaded"
        else:
            local_predicted_sign = "No Hand Detected" # Explicitly set if no hand

        # Buffer predictions for autocorrect
        current_time = time.time()
        if local_predicted_sign not in ["No Hand Detected", "Prediction Error", "Model not loaded", "Prediction Index Out of Bounds", "nothing"]:
            prediction_buffer.append(local_predicted_sign)

        if (current_time - last_buffer_process_time) > buffer_time:
            if prediction_buffer:
                most_common_letter = max(set(prediction_buffer), key=prediction_buffer.count)
                
                # Acquire lock before modifying global_collected_letters
                with frame_lock:
                    if most_common_letter == 'del':
                        if global_collected_letters:
                            global_collected_letters.pop()
                    elif most_common_letter == 'space':
                        global_collected_letters.append(' ')
                    elif most_common_letter not in ['nothing', 'del', 'space']: # 'nothing' means no action
                        global_collected_letters.append(most_common_letter)
                        global_letter_timeline.append({
            ...
            with frame_lock:
                sentence = "".join(global_collected_letters)
                global_autocorrected_sentence = str(TextBlob(sentence).correct())
        ...
        # Update global state variables for API access
        with frame_lock:
            global_predicted_sign = local_predicted_sign
            global_hand_detected = local_hand_detected
            global_fps = int(fps)
...
def video_feed():
    """Video streaming route. Generates frames to be sent to the frontend."""
    return Response(process_video_stream(), mimetype='multipart/x-mixed-replace; boundary=frame')

@app.route('/get_prediction')
def get_prediction():
    """Returns the current predicted sign, autocorrected sentence, hand detection status, and FPS."""
    with frame_lock: # Acquire lock to read global variables safely
        response_data = {
            'predicted_sign': global_predicted_sign,
            'autocorrected_sentence': global_autocorrected_sentence,
            'hand_detected': global_hand_detected,
            'fps': global_fps
        }
...
def submit_sentence():
    """Receives a sentence from the frontend and gets an AI response."""
    global global_ai_response_text, global_collected_letters, global_letter_timeline, global_autocorrected_sentence

    data = request.get_json()
    sentence = data.get('sentence', '').strip()

    if sentence:
        print(f"Submitting sentence to AI: '{sentence}'")
        global_ai_response_text = "Thinking..." # Update immediately for frontend feedback
        reply = get_small_talk_reply(sentence)
        with frame_lock: # Acquire lock before modifying global state after AI call
            global_ai_response_text = reply
            # Optionally clear collected letters and timeline after submission for a new conversation cycle
            global_collected_letters = []
            global_letter_timeline = []
            global_autocorrected_sentence = "" # Clear UI sentence after submission
        print(f"AI Reply: {global_ai_response_text}")
    else:
        global_ai_response_text = "No sentence provided for AI."
        print("No sentence provided for AI.")
    
    return jsonify({'ai_response': global_ai_response_text})

@app.route('/get_results_data')
def get_results_data():
    """Returns all data needed for the results page."""
    with frame_lock:
        return jsonify({
            'autocorrected_sentence': global_autocorrected_sentence,
            'ai_response': global_ai_response_text,
            'letter_timeline': global_letter_timeline
        })

@app.route('/manual_action', methods=['POST'])
def manual_action():
    """Handles manual actions like backspace or space from the frontend."""
    global global_collected_letters, global_autocorrected_sentence, global_letter_timeline
    ...
    with frame_lock:
        if action == 'backspace':
            if global_collected_letters:
                global_collected_letters.pop()
            # Optionally remove last item from timeline if it was the last letter
            if global_letter_timeline and global_letter_timeline and \
               (global_collected_letters == [] or global_collected_letters[-1] != global_letter_timeline[-1]['letter']):
                 global_letter_timeline.pop() # This is a simple logic, might need refinement
        elif action == 'space':
            global_collected_letters.append(' ')
            global_letter_timeline.append({
                'letter': '[SPACE]',
    ...
    return jsonify({'status': 'success', 'autocorrected_sentence': global_autocorrected_sentence})

@app.route('/start_webcam_backend', methods=['POST'])
def start_webcam_backend():
    """Initializes the webcam and starts the video processing thread on the backend."""
    global cap, video_stream_active, video_processing_thread, global_collected_letters, global_letter_timeline, global_autocorrected_sentence, global_ai_response_text, global_predicted_sign

    if video_processing_thread and video_processing_thread.is_alive():
        return jsonify({'status': 'info', 'message': 'Webcam already running.'})

    cap = cv2.VideoCapture(0)
    if not cap.isOpened():
        return jsonify({'status': 'error', 'message': 'Could not open webcam. Check if it\'s in use or connected.'}), 500
    ...
    global_autocorrected_sentence = ""
    global_ai_response_text = ""
    global_predicted_sign = "No Hand Detected"

    video_processing_thread = threading.Thread(target=process_video_stream)
    video_processing_thread.daemon = True # Allows the main program to exit even if thread is running
    video_processing_thread.start()
    print("Webcam started and video processing thread launched.")
    return jsonify({'status': 'success', 'message': 'Webcam started.'})

@app.route('/stop_webcam_backend', methods=['POST'])
def stop_webcam_backend():
    """Signals the video processing thread to stop and releases the webcam."""
    global video_stream_active, cap, video_processing_thread
    
    if not video_stream_active:
        return jsonify({'status': 'info', 'message': 'Webcam already stopped.'})

    video_stream_active = False
    if video_processing_thread and video_processing_thread.is_alive():
        video_processing_thread.join(timeout=5) # Wait for thread to finish
        if video_processing_thread.is_alive():
            print("Warning: Video processing thread did not terminate gracefully.")
    
    with frame_lock: # Acquire lock to ensure safe release
        if cap:
            cap.release()
            cap = None
...
# Route to serve the GIF file
@app.route('/<filename>')
def serve_static(filename):
    """Serves static files like the GIF from the current directory."""
...
<body>
  <div id="home-page">
   ...
   <main>
      ...
      <img class="hero-image" src="ASL Avatar.gif" alt="AI Sign Avatar" />
    </main>
    <footer>
  ...
  <script>
    ...
    const API_BASE_URL = window.location.origin; // Dynamically get current origin (e.g., http://127.0.0.1:5000)
...
if __name__ == '__main__':
    if knn_model is None:
        print("Model (knn_asl_model.pkl) not loaded. Please ensure it's in the same directory and valid.")
        # Decide if you want to exit or run without prediction functionality
        # exit()
    app.run(host='0.0.0.0', port=5000, debug=True, threaded=True) # debug=True for development, threaded=True